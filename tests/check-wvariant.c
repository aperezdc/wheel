...
START_TEST (test_wvariant_store_vals)
{
    ...
    var = w_variant_new (W_VARIANT_TYPE_INVALID);
    ck_assert_int_eq (W_VARIANT_TYPE_INVALID, w_variant_type (var));
    w_obj_unref (var);

    var = w_variant_new (W_VARIANT_TYPE_BOOL, true);
    ck_assert_int_eq (W_VARIANT_TYPE_BOOL, w_variant_type (var));
    ck_assert_int_eq (true, w_variant_bool (var));
    w_obj_unref (var);

    var = w_variant_new (W_VARIANT_TYPE_BOOL, false);
    ck_assert_int_eq (W_VARIANT_TYPE_BOOL, w_variant_type (var));
    ck_assert_int_eq (false, w_variant_bool (var));
    w_obj_unref (var);

    var = w_variant_new (W_VARIANT_TYPE_STRING, "Hello, world!");
    ck_assert_int_eq (W_VARIANT_TYPE_STRING, w_variant_type (var));
    ck_assert_str_eq ("Hello, world!", w_variant_string (var));
    w_obj_unref (var);

    w_buf_append_str (&buf, "Welcome Mr. Marshall");
    var = w_variant_new (W_VARIANT_TYPE_BUFFER, &buf);
    w_buf_clear (&buf);
    ck_assert_int_eq (W_VARIANT_TYPE_STRING, w_variant_type (var));
    ck_assert_str_eq ("Welcome Mr. Marshall", w_variant_string (var));
    w_obj_unref (var);

    var = w_variant_new (W_VARIANT_TYPE_NULL);
    ck_assert_int_eq (W_VARIANT_TYPE_NULL, w_variant_type (var));
    w_obj_unref (var);

    list = w_list_new (false);
    var = w_variant_new (W_VARIANT_TYPE_LIST, list);
    ck_assert_int_eq (W_VARIANT_TYPE_LIST, w_variant_type (var));
    ...
    var = w_variant_new (W_VARIANT_TYPE_DICT, dict);
    ck_assert_int_eq (W_VARIANT_TYPE_DICT, w_variant_type (var));
...
START_TEST (test_wvariant_mutate)
{
    ...
    w_variant_t *var = w_variant_new (W_VARIANT_TYPE_INVALID);
